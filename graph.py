...
def get_ddirect(self, ids):
    # input: int: id1 and id2
    # output: tuple: (from_id, to_id)
    # year(from_id) >= year(to_id)
    year_id1 = data.data_node_info["year"][data.data_node_info["id"] == ids[0]][0]  # TODO: risky because id may not unique
    year_id2 = data.data_node_info["year"][data.data_node_info["id"] == ids[1]][0]
    print(year_id1)
    print(year_id2)
...
data.data_train[["id_source", "id_target"]].iloc[0:2].apply(get_ddirect, axis=1)
